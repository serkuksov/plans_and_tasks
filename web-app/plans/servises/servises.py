...
def get_fixed_params_date(days: str, months: str, years: str) -> dict():
    """Преобразовать строковое предстовление фиксированой даты полученое в виде строкового представления числа без ареф аргументов.
...
def set_fixed_args_date(date: datetime.datetime, days: int, months: int, years: int) -> datetime.datetime:
    """Устанавливает постоянные атрибуты даты. Если значение фиксированого дня больше количества дней в месяце выставляется последний день месяца"""
    if months > 0:
...
def get_data_with_working_day(date: datetime.datetime) -> datetime.datetime:
    """Возвращает переданую дату или дату последнего предшествующего рабочего дня если попало на выходные"""
    cal = Russia()
...
def get_completion_date_for_task(completion_date: datetime.datetime, days: str, months: str, years: str) -> datetime.datetime:
    """Выдает дату завершения выполнения задачи на основе даты завершения плана и параметров смещения"""
